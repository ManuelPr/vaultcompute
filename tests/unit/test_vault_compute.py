...
from vaultcompute.ports.sandbox import SandboxError, SandboxTimeoutError
    ...
    for hours_ago in (5, 4, 3, 2, 1):
        store.put(
            VaultRecord(
                ...
                lineage=Lineage(op="vault_compute", inputs=("⟦tok_00000001⟧",)),
                policy=Policy(),
            )
        )
