...
Codex continues its reasoning. It reuses Blindfold's local SQLite vault and the
...
1. Install the repository so `blindfold` is on `PATH`.
2. Put `blindfold.yaml` in the directory from which Codex starts.
...
Blindfold therefore does not claim to protect those paths.

For a configured hooked tool, an unreadable or non-matching result is replaced
with a safe Blindfold error instead of being passed through. For a tool path
