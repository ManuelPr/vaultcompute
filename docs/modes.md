...
VaultCompute has four ways to plug in. They share the same token vault and differ
in two questions that decide which one you want: **which tool results the host
lets VaultCompute intercept**, and **who owns the model's final answer**, therefore
...
## What VaultCompute does, in one paragraph

When your agent calls a tool, the tool's answer normally goes straight into the
model's context and therefore to your LLM provider. VaultCompute intercepts that
...
For a declared table it calls the controlled `vault_table` query tool and
...
- **Mode B**: your `VaultComputeSession` holds it and renders the final answer. Works.
...
- `vault_table` is added when a table is declared and the controlled profile
  is active. `vault_compute` is added only for `compute.mode: python_unsafe`.

**What you do not get**

- **Rehydration**, unless the client is yours and you teach it to call the
  custom `vaultcompute/rehydrate` JSON-RPC method. No third-party client does.
...
**What it is.** You create a `VaultComputeSession` around the agent loop you
...
config = load_config("vaultcompute.yaml")
session = VaultComputeSession(config, session_id=f"user_{user_uuid}")
```
...
3. **Advertise only the selected operation tools.** In the default controlled
   profile add `vault_table` for declared tables. Add
   `vault_compute.build_tool_definition()` only after an explicit
...
Code calls VaultCompute at the right moments; nothing sits in the middle.

**Setup.**

```bash
uv tool install .            # puts `vaultcompute` on PATH
claude --plugin-dir ./plugin
```

with a `vaultcompute.yaml` in the directory you start from:
...
| `PreToolUse` hook | Denies a configured built-in before execution when VaultCompute has no tested adapter for that result shape |
...
| `vaultcompute` MCP server | Offers only the operations selected by `compute.mode`; Python is absent by default |
...
- **A memory vault will not work.** Every hook run is a separate process and the
  MCP server is a third one, so the vault must be a shared file. VaultCompute
  refuses to run the hooks with `backend: memory` rather than minting
  placeholders nobody can resolve.
- **`vaultcompute` must be on `PATH`** for every hook process and for the server.
...
VaultCompute cannot tell which part of a free-text answer is sensitive, and letting
...
[`plugins/vaultcompute-codex/`](../plugins/vaultcompute-codex/). Codex invokes
`PostToolUse` after a supported local tool completes. VaultCompute tokenizes the
...
VaultCompute uses Codex's `continue: false` post-tool response rather than a
...
| `vault_compute` on single values | automatic | you wire it | automatic | automatic |
...
```bash
vaultcompute audit ~/.claude/projects/<your-project>/<session>.jsonl
```
...
of it and nothing to look for. The audit measures whether VaultCompute kept the
...
- **`vault_compute` runs arbitrary Python**, so a model actively trying to
  extract a value can learn one bit per call by writing code that fails on
  purpose. `vault_table` cannot be used that way — where your data is a
  list, prefer a table. For `vault_compute`, `compute.max_calls_per_token`/
  ...
  and reported by `vaultcompute audit`, but the limit still only slows the channel.
- **Access control is your API's job.** VaultCompute forwards requests untouched.
  If your API answers anyone, VaultCompute faithfully hides data the caller should
  never have received.
...
`VAULTCOMPUTE_VAULT_KEY`; there is deliberately no way to put the key in the config
file it protects.
