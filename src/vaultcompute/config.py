...
from vaultcompute.core.tokenizer import SchemaField, path_segments, validate_path
...
class ComputeConfig(StrictConfigModel):
    #: ``controlled`` exposes only vault_table. ``python_unsafe`` also
    #: exposes arbitrary Python and must be a deliberate opt-in. ``disabled``
    #: exposes neither compute surface.
    mode: str = "controlled"
    #: A root secret lineage used by `vault_compute` more than this many
    ...
    #: "`vault_compute` can reveal one bit per call") can extract an exact value
...
def describe_config(config: VaultComputeConfig) -> str | None:
    ...
    for tool_name in sorted(config.schemas):
        ...
        for path, schema in tables:
            ...
            lines.append(f"      query it with vault_table; columns: {schema.describe()}")
    if not lines:
        return None
    return (
        "VaultCompute is protecting some of this session's tool results.\n\n"
        + PLACEHOLDER_PROMPT
        + (
            "\n\nArbitrary Python is enabled by explicit operator opt-in and may "
            "appear as `mcp__vaultcompute__vault_compute`."
...
def build_token_store(config: VaultComputeConfig) -> TokenStore:
...
def required_table_paths_for(config: VaultComputeConfig, tool_name: str) -> set[str]:
    tool = config.schemas.get(tool_name)
    if tool is None:
        return set()
...
def table_schemas_for(config: VaultComputeConfig, tool_name: str) -> list[tuple[str, TableSchema]]:
