...
while host-specific event contracts live under :mod:`vaultcompute.hosts`.
"""
...
from vaultcompute.hosts import CLAUDE_CODE, CODEX, HOSTS, claude_code, codex
...
def handle_post_tool_use(
    event: dict, *, config: VaultComputeConfig, store: TokenStore
) -> dict | None:
...
def failure_response(host: str, event_name: str, message: str) -> dict | None:
    ...
    if host == CLAUDE_CODE and event_name == PRE_TOOL_USE:
        return {
            "hookSpecificOutput": {
                ...
                "permissionDecisionReason": f"VaultCompute: {message}",
            }
        }
    ...
    return {"continue": False, "stopReason": f"VaultCompute: {message}"}
