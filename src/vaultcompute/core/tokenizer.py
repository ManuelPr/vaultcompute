...
from vaultcompute.core.lineage import Lineage, Policy, TableSchema, VaultRecord
...
def describe_tables(tables: list[tuple[str, TableSchema]]) -> str | None:
    ...
    return (
        "VaultCompute: the lists at these paths come back as a single "
        "⟦tok_…⟧ placeholder each, hiding every row. You cannot read them. "
        "Use the vault_table tool to filter, sort, aggregate or select over "
...
def describe_schema(
    ...
    operation_note = (
        "pass a placeholder to vault_compute to operate on it"
        if allow_python_compute
        else "the controlled profile does not let the model compute on these scalar placeholders"
    )
    return (
        "VaultCompute: values at these paths come back as ⟦tok_…⟧ placeholders, "
