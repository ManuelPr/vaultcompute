"""`vault_table` — query a collective token without seeing its rows.

The counterpart to `vault_compute`, and deliberately weaker: the model
...
from vaultcompute.core.table import COMPARISONS, ROW_OPS, TERMINAL_OPS, run_query
...
VAULT_TABLE_TOOL_NAME = "vault_table"
...
def handle_vault_table(
    ...
    *,
    ...
    if not isinstance(token, str) or not isinstance(ops, list):
        raise ValueError("vault_table requires a string `table` and a list `ops`")
    ...
    if record.table is None:
        raise ValueError(
            f"{token} is not a table token; use vault_compute for single values"
        )
