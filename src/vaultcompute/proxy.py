...
def _error_message(request_id: Any, detail: str) -> dict:
    return {
        "jsonrpc": "2.0",
        "id": request_id,
        "error": {"code": -32001, "message": f"VaultCompute blocked a response: {detail}"},
    }
...
async def run_proxy(downstream_cmd: list[str], config_path: Path | None = None) -> None:
    ...
    try:
        ...
        if died_on_its_own and child.returncode != 0:
            ...
            print(
                f"[vaultcompute] downstream {downstream_cmd[0]!r} exited with code "
    ...
    while True:
        ...
        if not isinstance(msg, dict):
            if state.config.proxy.strict:
                print("[vaultcompute] blocked a JSON-RPC batch in strict mode", file=sys.stderr)
                ...
                continue
            print("[vaultcompute] forwarding a JSON-RPC batch in permissive mode", file=sys.stderr)
        ...
        if method == "tools/call":
            ...
            if params.get("name") == VAULT_COMPUTE_TOOL_NAME:
                ...
                await asyncio.to_thread(_handle_vault_compute, msg, write_client, state)
                continue
            if params.get("name") == VAULT_TABLE_TOOL_NAME:
                ...
                _handle_vault_table(msg, write_client, state)
                continue
...
async def _pump_child_to_client(child, write_client, state: ProxyState) -> None:
    ...
    while True:
        ...
        if not isinstance(msg, dict):
            if state.config.proxy.strict:
                print("[vaultcompute] blocked a downstream JSON-RPC batch in strict mode", file=sys.stderr)
                write_client(
        ...
        try:
            ...
            if state.config.proxy.strict:
                ...
                print(f"[vaultcompute] permissive passthrough: {exc}", file=sys.stderr)
...
def _tokenize_resource_read(msg: dict, requested_uri: str, state: ProxyState) -> None:
    ...
    for part in contents:
        ...
        if not isinstance(text, str):
            ...
            print(
                f"[vaultcompute] {uri} declares protected paths but returned no text part",
                file=sys.stderr,
            )
        ...
        try:
            ...
            print(
                f"[vaultcompute] {uri} declares protected paths but did not return JSON",
                file=sys.stderr,
            )
...
def _handle_vault_table(msg: dict, write_client, state: ProxyState) -> None:
    args = ((msg.get("params") or {}).get("arguments")) or {}
    try:
        new_token = handle_vault_table(
            args,
            store=state.store,
            policy=state.policy,
            session_id=state.session_id,
        ...
        payload = {
            "jsonrpc": "2.0",
            "id": msg.get("id"),
            "result": {"content": [{"type": "text", "text": f"vault_table error: {exc}"}], "isError": True},
        }
...
def _handle_vault_compute(msg: dict, write_client, state: ProxyState) -> None:
    args = ((msg.get("params") or {}).get("arguments")) or {}
    try:
