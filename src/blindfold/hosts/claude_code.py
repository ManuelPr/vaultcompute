...
from blindfold.config import BlindfoldConfig, describe_config
from blindfold.core.rehydrator import TOKEN_PATTERN, rehydrate
from blindfold.hosts.common import (
    ProtectionError,
    is_protected,
    protect_json_text,
    safe_reason,
...
def handle_pre_tool_use(event: dict, *, config: BlindfoldConfig) -> dict | None:
    """Deny configured built-ins whose result shape Blindfold cannot replace."""
    ...
    return {
        "hookSpecificOutput": {
            ...
            "permissionDecisionReason": safe_reason(
                tool_name,
                "this Claude Code output shape has no audited Blindfold adapter",
            ),
        }
    }


def _protect_text(text: str, event: dict, config: BlindfoldConfig, store: TokenStore):
...
def _mcp_replacement(
    response: Any, event: dict, config: BlindfoldConfig, store: TokenStore
) -> Any:
