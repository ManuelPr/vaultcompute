...
VaultCompute intercepts **tool results**, not the user's original question. If the user types *"What is James Brown's salary?"*, the name and the intent go to the LLM provider in cleartext. Only the tool response gets tokenized.
...
- ~~**`vaultcompute audit` flagged a `vault_compute` result as a leak when it was really a name the model already knew.**~~ **Closed.** `vault_compute` mints a fresh token for `result` unconditionally — even when the code is something like `result = 'James Brown' if resolve(a) > resolve(b) else 'John Smith'`, where the actual secret is *which* name won, not the names themselves, both already known to the model and typed into its own code. That literal sits in the transcript, in the tool call's own arguments, before the derived token even exists. `audit`'s substring match couldn't tell "the model wrote this itself" from "this came out of the vault," and reported the former as a leak. Fixed by scanning the transcript for string literals inside any `vault_compute` call's `code` argument and excluding a `vault_compute`-lineage match against that set — reported separately as "explained," not silently dropped. A value actually produced by `resolve(...)` arithmetic (a summed salary, say), never typed as a literal anywhere, is unaffected and still flagged.
